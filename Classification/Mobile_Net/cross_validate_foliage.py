...
def train_one_fold(train_dir, test_dir, args, fold_idx):
    ...
    # Only create EMA if not disabled (EXACT same as main.py)
    ...
    for epoch in range(args.epochs):
        ...
        # Validate - use EMA model if available (EXACT same as main.py)
        if args.disable_ema:
            val_model = model.module  # Unwrap DataParallel to get the actual model
        else:
            val_model = model_ema.model
        
        # NOTE: main.py's validate() function does NOT call model.eval()!
        # EMA model is already in eval mode from initialization
        # When disable_ema=True, model stays in train mode (using batch stats for BN)
